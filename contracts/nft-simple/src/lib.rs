...
impl Contract {
    #[init]
    pub fn new(owner_id: AccountId, metadata: NFTMetadata, supply_cap_by_type: TypeSupplyCaps, locked: Option<bool>) -> Self {
        let mut this = Self {
    ...
    fn measure_min_token_storage_cost(&mut self) {
        ...
        let u = UnorderedSet::new(
            StorageKey::TokenPerOwnerInner {
                account_id_hash: hash_account_id(&AccountId::new_unchecked(tmp_account_id.to_string())),
            }
            .try_to_vec()
            .unwrap(),
        );
        self.tokens_per_owner.insert(&AccountId::new_unchecked(tmp_account_id.to_string()), &u);
        ...
        self.tokens_per_owner.remove(&AccountId::new_unchecked(tmp_account_id.to_string()));
    }
