...
impl Contract {
    ...
    pub fn update_price(
        ...
        if !self.ft_token_ids.contains(&AccountId::new_unchecked(ft_token_id.to_string())) {
            env::panic_str(format!("Token {} not supported by this market", ft_token_id).as_str());
        }
    ...
    pub fn offer(&mut self, nft_contract_id: AccountId, token_id: String) {
        ...
        let price = sale
            .sale_conditions
            .get(&AccountId::new_unchecked(ft_token_id.to_string()))
        ...
        if !sale.is_auction && deposit == price {
            self.process_purchase(
                contract_id,
                token_id,
                AccountId::new_unchecked(ft_token_id.to_string()),
                U128(deposit),
                buyer_id,
            );
        } else {
    ...
    pub fn accept_offer(
        ...
        let mut sale = self.sales.get(&contract_and_token_id).expect("No sale");
        let bids_for_token_id = sale.bids.remove(&AccountId::new_unchecked(ft_token_id.to_string())).expect("No bids");
            ...
            near_sdk::serde_json::from_slice::<Payout>(&value)
                ...
                .and_then(|payout| {
                    ...
                    if payout.len() + sale.bids.len() > 10 || payout.is_empty() {
                        env::log_str(format!("Cannot have more than 10 royalties and sale.bids refunds").as_str());
                        None
                    } else {
